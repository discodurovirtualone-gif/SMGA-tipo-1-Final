...
app.patch("/api/registros_basicos/:id_vaca/potencial", async (req, res) => {
  try {
    ...
    await db
      ...
      .where(eq(registrosBasicos.id_vaca, id_vaca));
    res.json({ ok: true, id_vaca, potencial_vaca });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Failed to patch potencial_vaca" });
  }
});
